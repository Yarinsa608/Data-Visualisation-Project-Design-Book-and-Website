const drawPieChart = (data) => {
  ...
  const color = d3.scaleOrdinal()
    ...
    .range(["red", "blue", "orange", "yellow", "cyan"]);
  ...
  chartGroup.selectAll("path")
    ...
    .on("mouseover", function (event, d) {
      ...
      tooltip.style("opacity", 1)
        .html(`<strong>${d.data.ageGroup}</strong><br/>${Math.round((d.data.totalFines / totalFines) * 100)}%`)
  ...
  legend.selectAll("text")
    ...
    .style("fill", "#333")
    .text(d => `${d.ageGroup} - ${Math.round((d.totalFines / totalFines) * 100)}%`);
};
